...
app.post('/login',(req,res)=>{
  ...
  allUsers = JSON.parse(allUsers)
  if(allUsers.users) {
    console.log('data found ',allUsers.users)
    var userFoundData = allUsers.users.filter(obj => { console.log(obj); return obj.email == useremail })
    console.log("userFoundData >> ",userFoundData);
    if(userFoundData.length) {
      if(userpass == userFoundData[0].password){
        ...
        return res.status(200).json({success:1,loginErrMsg:"",userList:userList})
